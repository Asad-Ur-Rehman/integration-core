...
lazy val core = (project in file("."))
  .settings(
    ...
    releaseProcess := {
      if (nexusUser == defaultUser) {
        println(s"********* A required environment variable is not set 'NEXUS_REPO_USER'")
      }
      if (nexusPassword == defaultPassword) {
        println(s"********* A required environment variable is not set 'NEXUS_REPO_PASSWORD'")
      }
