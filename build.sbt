...
lazy val core = (project in file("."))
  .settings(
    ...
    releaseVersion := {
      ver =>
        isFinal match {
          case "" => Version(ver).map(_.withoutQualifier.string).getOrElse(versionFormatError)
    ...
    releaseNextVersion := { ver =>
      isFinal match {
        case "" => Version(ver).map(_.bump.string).getOrElse(versionFormatError)
